...
contract Projects {
    ...
    mapping(address => PrivilegedUser) public specialUsers;
    mapping(bytes32 => uint256) public projectIndex;

    /// @dev checks if the Privileged user's address provided is a valid Priviliged user or not
    /// @param _user which is the address of the Privileged user
    /// @return userValidity of type bool. Returns true if the user is valid, false otherwise
    function isValidPrivilegedUser(address _user) public view returns (bool) {
        PrivilegedUser memory user = specialUsers[_user];
        return user.name.length > 0 && isValidProject(user.project_name);
    }
